...
\item{(PD)} \emph{Primary demands.}
	...
	\item\label{PD2:disjoint}  For any two different primary demands $\kappa,\kappa'\in P$ we have
				$\wbarN(\kappa)\cap \wbarN(\kappa') = \emptyset$.

	\item \label{PD2:yi} For each site $i\in\sitesset$, 
		$ \sum_{\mu\in i}\sum_{\kappa\in P}\barx_{\mu\kappa} \leq y_i^\ast$.
		(Recall that $\barx_{\mu\kappa}\neq 0$ iff $\mu\in \wbarN(\kappa)$.)
		
	\item \label{PD2:assign} Each demand $\nu\in\demandset$ is assigned
...
demands. The process completes when all clients are exhausted.
...
\begin{algorithm}[ht]
  ...
  \begin{algorithmic}[1]
    ...
    \If{$\wtildeN_1 (p)\cap \wbarN(\kappa) \neq \emptyset$ for some primary demand $\kappa\in P$}
...
equal to the old value $\bary_{\mu}$. 
At the same time, the algorithm does a split whenever necessary to preserve
properties (c1) and (c2), and after the completion (c1) will imply Property~\ref{P1:complete}.  
Finally, to justify
...
the neighborhood of $\kappa$.  Thus Property~\ref{P2:disjoint} follows.
...
Lemmas~\ref{lem: property P2:yi holds},~\ref{lem: property
...
and $\wbarN(\kappa_q)$ is disjoint from
$\wbarN(\nu_s)$. By the time when we add facilities using
\AugmentToUnit() in Line~19--21 of Pseudocode~\ref{alg:lpr2}, 
the neighborhoods of all primary demands, including  $\wbarN(\kappa_1), \ldots,
\wbarN(\kappa_{r_j})$, are already fixed. Further, these added facilities
do not appear in any neighborhood $\wbarN(\kappa_1), \ldots, \wbarN(\kappa_{r_j})$ and
each of these facilities is added to the neighborhood of exactly one sibling.
Therefore all the disjointness conditions are preserved after the augment step,
proving Property~\ref{P2:diff}.
\end{proof}
