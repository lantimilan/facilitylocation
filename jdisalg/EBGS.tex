...
We now return to the estimate of connection cost. Let $C_{\nu}$ be the
random variable for the connection cost of a demand $\nu$ and let
$\Exp[C_{\nu}]$ be the expected value of $C_{\nu}$. If $\nu$ is a
primary demand, due to the algorithm, we have $\Exp[C_{\nu}] =
\clsdist(\nu)$, which easily satisfies the bound we need in the final
total connection cost estimation. For the rest of this section we
consider $\nu$ being a non-primary demand.

To bound $\Exp[C_{\nu}]$, we consider the conditional expected
distances in three cases, along with the associated probabilities,
when $\nu$ is connected to its close facility in $\wbarclsnb(\nu)$, or
its far facility in $\wbarfarnb(\nu)$, or its target facility
$\phi(\kappa)$ where $\kappa$ is the primary demand that $\nu$ is
assigned to and $\phi(\kappa)$ is the only facility open in
$\wbarclsnb(\kappa)$.

We start with a lemma for the third case when $\nu$ connects to
$\phi(\kappa)$. The lemma provides an estimate of the cost of this
indirect connection, conditioned on the event that no facility in
$\wbarN(\nu)$ opens. Note that an indirection can only occur when no
neighboring facility of $\nu$ opens.  The proof of this lemma relies
on Properties~(PD'.\ref{PD1:assign:overlap}) and
...
\begin{lemma}\label{lem: EBGS target connection cost}
  Let $\nu$ be a non-primary demand assigned to $\kappa\in P$ and let
  $\Lambda^{\nu}$ be the event that some facility in $\wbarN(\nu)$
  opens in Algorithm~{\EBGS}'s integral solution, and $\neg
  \Lambda^\nu$ be the complement event. Then
%
\begin{equation*}
\Exp[C_{\nu} \mid \neg \Lambda^{\nu}] \leq
  \clsdist(\nu) + \clsmax(\nu) + \fardist(\nu).
...
$A$ is an arbitrary set of facilities.

\begin{lemma}
  \label{lem: expected distance in EBGS}
  Given a set $A$ of facilities, and a demand $\nu$, let
  $\Lambda^\nu_A$ be the event that at least one facility is open in
  Algorithm {\EBGS}'s integral solution. Then the expected distance
  from $\nu$ to the nearest open facility in $A$, conditioned on event
  $\Lambda^\nu_A$ occurs, is at most $D(A,\nu) = \sum_{\mu \in A}
...
\begin{proof}
  ...
  this conditional expectation, we use a provably worse random process
  to make connections. Specifically, we group facilities in $A$ by the
  primary demand's neighborhood that they fall in. Facilities not in
  any primary demand's neighborhood forms a singleton group by
  itself. Then each group can have at most one facility open and the
  event of facility opening betwen groups are independent. We connect
  to the facility opened in the group $G$ with minimum $D(G, \nu)$,
  the average distance to demand $\nu$. As before we denote the groups
  by $G_1, \ldots, G_k$ and $\bard_s = D(G_s, \nu) = \sum_{\mu \in
    G_s} d_{\mu\nu} \bary_{\mu} / \sum_{\mu \in G_s} \bary_{\mu}$ with
  $\bard_1 \leq \ldots \leq \bard_k$. The only difference is that we
  now have $g_s = \sum_{\mu \in G_s} \gamma \bary_{\mu}$ because the
  algorithm opens a facility $\mu$ with probability
  $\gamma\bary_{\mu}$. Denote the conditional expectation as
  $\Exp[d(A,\nu) \mid \Lambda^\nu_A]$, we have
  \begin{align}
    \Exp[d(A,\nu) \mid \Lambda^\nu_A] &\leq \frac{1}{\Prob[A]} (\bard_1 g_1 +
    \bard_2 g_2 (1 - g_1) + \ldots + \bard_k  g_k(1 -
    g_1)\ldots(1-g_{k-1}))
    \label{eqn: dist set to nu 1}
    \\
    &\leq \frac{1}{\Prob[A]} \frac{\sum_{s=1}^k \bard_s
    ...
    \notag
  \end{align}
Inequality (\ref{eqn: dist set to nu 1}) follows from the use of a
provably worse random process to make connections. Inequality
(\ref{eqn: dist set to nu 2}) follows by using inequality (\ref{eq:min
  expected distance}). The last few equalities follow from the
definition of $\bard_s, g_s$ and $D(A,\nu)$.
\end{proof}

We need two notations for the following corollary. Recall that
$\Lambda^\nu$ is the event that some facility in $\wbarN(\nu)$ opens.
We also introduce notation $\Lambda^\nu_{\cls}$ for the event that
some facility in $\wbarclsnb(\nu)$ opens. Then
\begin{corollary}
...
\begin{proof}
  ...
  the proof follows from Lemma~\ref{lem: expected distance in
    EBGS}. By setting the set $A$ to $\wbarclsnb(\nu)$, we have
  \begin{equation*}
    \Exp[C_{\nu} \mid \Lambda_{\cls}^\nu] \leq D(\wbarclsnb(\nu), \nu)
    = \clsdist(\nu),
  \end{equation*}
  and by setting the set to $\wbarfarnb(\nu)$, we have 
  \begin{equation*}
    \Exp[C_{\nu}
    \mid \Lambda^\nu \wedge \neg \Lambda_{\cls}^\nu] \leq
    D(\wbarfarnb(\nu), \nu) = \fardist(\nu).    
...
%
  \begin{align*}
    \Exp[C_{\nu}] 
		...
		\\
		&\leq  \clsdist(\nu) \cdot \Prob[\Lambda^\nu_{\cls}]
			+ \fardist(\nu)	\cdot \left(\Prob[\Lambda^\nu] -
                        \Prob[\Lambda^\nu_{\cls}]\right)\\
                        &\quad\quad
			+ [\clsdist(\nu) + \clsmax(\nu) + \fardist(\nu)] \cdot (1 - \Prob[\Lambda^\nu])
		\\
		&\leq  \clsdist(\nu) \cdot \Prob[\Lambda^\nu_{\cls}]
			+ \fardist(\nu)	\cdot \left(\Prob[\Lambda^\nu] -
                        \Prob[\Lambda^\nu_{\cls}]\right)\\
			...
			+ [\clsdist(\nu) + \fardist(\nu) + \fardist(\nu)] \cdot (1 - \Prob[\Lambda^\nu])
		\\
                &= (\clsdist(\nu) + \fardist(\nu))\cdot (1-\Prob[\Lambda^\nu]) - (\fardist - \clsdist) \cdot \Prob[\Lambda^\nu_{\cls}] + \fardist(\nu).
\end{align*}
The first inequality follows from Corollary~\ref{coro: EBGS close and
  far distance} and Lemma~\ref{lem: EBGS target connection cost}. The
second inequality follows from $\clsmax(\nu) \leq \fardist(\nu)$. We
then bound the corresponding probabilities $\Prob[\Lambda^\nu_{\cls}]$
and $\Prob[\Lambda^\nu]$ with the following lemma.

\begin{lemma}
  \label{lem: close neighbor probability}
  The probability that at least one facility in $\wbarclsnb(\nu)$
  opens, that is $\Prob[\Lambda^\nu_{\cls}]$, is no less than $1-1/e$.
\end{lemma}
\begin{proof}
  To estimate $\Prob[\Lambda^\nu_{\cls}]$, we again consider the
  facilities in $\wbarclsnb(\nu)$ by grouping them according to the
  primary demand's neighborhood that they fall in. Facilities do not
  fall in any primary demand's neighborhood form a singleton group by
  itself. Then the event of facilit opening in each group is
  independent. Let the groups be $G_1, \ldots, G_k$, also let $g_s =
  \sum_{\mu \in G_s} \gamma \bary_{\mu}, s=1,\ldots,k$. It is easy to
  ...
  \bary_{\mu}$. Therefore the probability that at least one facility
  in all those groups opens is $1 - \prod_{s=1}^k (1 - g_s)$, which is
  at least $1 - e^{-\sum_{s=1}^k g_s} = 1 - 1/e$. Therefore we have
  $\Prob[\Lambda^\nu_A] \geq 1 - 1/e$.
\end{proof}

\begin{lemma}
  \label{lem: far neighbor probability}
  The probability that at least one facility in $\wbarN(\nu)$ opens,
  that is $\Prob[\Lambda^\nu]$, is no less than $1-1/e^\gamma$.
\end{lemma}
\begin{proof}
  Similar to the proof of Lemma~\ref{lem: close neighbor
    probability}. The probability $\Prob[\Lambda^\nu]$ is no less than
  $1 - e^{-\sum_{s=1}^k g_s} = 1 - 1/e^\gamma$ because we now have
  $\sum_{s=1}^k g_s = \gamma \sum_{\mu \in \wbarN(\nu)} \bary_{\mu} =
  \gamma \cdot 1 = \gamma$.
\end{proof}

 We now continue our derivation.
\begin{align*}
  \Exp[C_\nu]
             &\leq (\clsdist(\nu) + \fardist(\nu))\cdot (1-\Prob[\Lambda^\nu]) - (\fardist - \clsdist) \cdot \Prob[\Lambda^\nu_{\cls}] + \fardist(\nu)\\
