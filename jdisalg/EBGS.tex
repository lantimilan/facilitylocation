...
Using essentially the same argument as the proof of Lemma~\ref{lem:
  echs expected C_nu}, we have the following more general lemma.
...
\begin{lemma}
  ...
  the random variable for the set of facilities opened by the
  algorithm. Then the expected distance from $\nu$ to the nearest
  facility in $A'$, conditioned on that $A'$ is not empty, denoted as
  $\Exp[d(A',\nu) \mid A' \neq \emptyset]$, is at most $D(A,\nu) =
...
\begin{proof}
  ...
  $\Exp[d(A', \nu) \mid A']$, we use a provably worse random process
  ...
  $\gamma\bary_{\mu}$. Then we have the following derivation.
  \begin{align*}
    \Exp[d(A',\nu) \mid A'] &\leq \frac{1}{\Prob[A]} (\bard_1 g_1 +
    \bard_2 g_2 (1 - g_1) + \ldots + \bard_k  g_k(1 - g_1)\ldots(1-g_{k-1}))\\
    &\leq \frac{1}{\Prob[A]} \frac{\sum_{s=1}^k \bard_s
      g_s}{\sum_{s=1}^k  g_s} (1 - \prod_{s=1}^k (1 -  g_s))\quad\quad
    \text{use inequality (\ref{eq:min expected distance})}\\
...
\begin{proof}
  ...
  the proof follows from Lemma~\ref{lem: expected distance in EBGS} by
  considering the set $A$ as $\wbarclsnb(\nu)$ and then
  $\wbarfarnb(\nu)$.
\end{proof}
...
\begin{proof}
  ...
  to. Given that, we estimate $\Exp[C_\nu]$ as follows. Below we let
  $p_c \stackrel{\mathrm{def}}{=} \Prob[\Lambda_{\cls}^\nu]$ and $p_d
  \stackrel{\mathrm{def}}{=} \Prob[\Lambda^\nu]$. Then the expected
  connection cost of demand $\nu$ can be bounded as follows.
%
  \begin{align*}
    \Exp[C_{\nu}] 
...
The first inequality follows from Corollary~\ref{coro: EBGS close and
  ...
  EBGS target connection cost}. We then bound the corresponding
probabilities $p_c$ and $p_d$.
...
$G_1, \ldots, G_k$, also let $g_s = \sum_{\mu \in G_s} \gamma
\bary_{\mu}, s=1,\ldots,k$. It is easy to see that $\sum_{s=1}^k g_s =
\sum_{\mu \in \wbarclsnb(\nu)} \gamma \bary_{\mu} = 1$. For any group
$G_s$, the probability that a facility in this group opens is
$\sum_{\mu \in G_s} \gamma \bary_{\mu} = g_s$ because in the algorithm
at most one facility in a group can be chosen and each is chosen with
...
- g_s)$, which is at least $1 - e^{\sum_{s=1}^k g_s} = 1 -
