...
%
\end{lemma}
A sketch of the proof is in \ref{sec: proof of lemma 15}.  We are now
ready to bound the overall connection cost of Algorithm~{\EBGS}.
...
\begin{proof}
  ...
  non-primary demand $\nu$. Let $\wbarN(\nu) =
  \{\mu_1,\ldots,\mu_h\}$, where the facilities are ordered by
  non-decreasing distance to $\nu$.  Thus, denoting $d_s =
  d_{\mu_s\nu}$, for all $s$, we have $d_1 \le d_2 \le ... \le
  d_h$. Choose $l$ such that $\wbarclsnb(\nu) =
  \{\mu_1,\ldots,\mu_l\}$ and $\wbarfarnb(\nu) = \{\mu_{l+1},
  \ldots,\mu_{h}\}$.

  Let $C_{\nu}$ be the random variable equal to the distance from
  $\nu$ to the facility that it connects to in the integral solution.
  As before, $\Lambda^\nu$ is the event that some facility in
  $\wbarN(\nu)$ opens, and we also use notation $\Lambda^\nu_{\cls}$
  for the event that some facility in $\wbarclsnb(\nu)$ opens. Using
  an argument similar to that in the analysis of Algorithm~{\ECHS} in
  Section~\ref{sec: 1.736-approximation}, we can show that
  $\Exp[C_\nu]$ is upper bounded by the expected connection cost of
  $\nu$ obtained by the alternative random process that opens each
  facility $\mu \in \wbarN(\nu)$ independently with probability
  $\gamma\bary_{\mu}$, and connects to the nearest facility in
  $\wbarclsnb(\nu)$ if one opens, otherwise connect to the nearest
  facility open in $\wbarfarnb(\nu)$, failing that, connect to its
  target facility $\phi(\kappa)$, the facility opened in the
  neighborhood of its assigned primary demand $\kappa$. Following this
  reasoning, we estimate $\Exp[C_\nu]$ as follows. Below we let $p_c
  \stackrel{\mathrm{def}}{=} 1-\prod_{s=1}^l (1-\gamma\bary_{\mu_s})$
  and $p_d \stackrel{\mathrm{def}}{=} 1-\prod_{s=1}^h
  (1-\gamma\bary_{\mu_s})$.
%
  \begin{align*}
    \Exp[C_{\nu}] 
...
\end{align*}
The first inequality follows from a similar argument in the proof of
Lemma~\ref{lem: echs expected C_nu}, and the second inequality is due
to Lemma~\ref{lem: EBGS target connection cost}.

By definition $p_c = 1 - \prod_{s=1}^l (1-\gamma\bary_{\mu_{s}}) \geq
1-1/e$ since $\sum_{s=1}^l \bary_{\mu_{s}} = 1/\gamma$. Similarly $p_d
= 1 - \prod_{s=1}^h (1 - \gamma\bary_{\mu_s}) \geq 1 - 1/e^\gamma$
since $\sum_{s=1}^h \bary_{\mu_s} = 1$. Therefore,
\begin{align*}
  \Exp[C_\nu]
             &\leq (\clsdist(\nu) + \fardist(\nu))\cdot (1-p_d) - (\fardist - \clsdist) \cdot p_c + \fardist(\nu)\\
