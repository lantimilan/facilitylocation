...
total connection cost estimation. We now estimate the expected
...
$\Lambda^\nu$ is the event that at least one facility in $\wbarN(\nu)$
opens.  We also introduce notation $\Lambda^\nu_{\cls}$ for the event
that at least one facility in $\wbarclsnb(\nu)$ opens. Then
\begin{corollary}
...
\begin{proof}
  ...
  \begin{equation*}
    ...
    = \clsdist(\nu),
  \end{equation*}
  and by setting the set $A$ to $\wbarfarnb(\nu)$, we have
  \begin{equation*}
    \Exp[C_{\nu}
    \mid \Lambda^\nu \wedge \neg \Lambda_{\cls}^\nu] \leq
    D(\wbarfarnb(\nu), \nu) = \fardist(\nu).    
...
and $\Prob[\Lambda^\nu]$ with the following two lemmas.
