...
each client $j=1,2,3,4$. When computing $\wtildeN_1(2)$, facility
$\dot{1}$ will get split into $\dot{1}$ and $\ddot{1}$ with
$\bary_{\dot{1}}=1/3$ and $\bary_{\ddot{1}} = 1$. (This will happen in
...
$\wbarN(4') = \{\dot{2}, \dot{3}\}$. 
In the fifth iteration we  pick client $2$ again.
This time we have $\wtildeN_1(2) = \{\ddot{1}\}$, and this set is
disjoint from $\wbarN(1')$. So we create demand $2''$ and
make it primary, and set $\wbarN(2'') = \{\ddot{1}\}$. In the next two
iterations we create one demand for each client $3$ then $4$, denoted $3''$ and $4''$,
respectively. They both get assigned to $2''$, because their neighborhood
$\wtildeN_1(j)$ overlaps with
$\wbarN(2'')$. We also set $\wbarN(3'') = \{\ddot{1}\}$ and
$\wbarN(4'') = \{\ddot{1}\}$. 
