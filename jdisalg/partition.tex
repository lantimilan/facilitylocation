...
\begin{subtable}{0.4\textwidth}
  ...
  \begin{tabular}{c | c c c c c c c | c} % seven demands, five facilities
    ...
    \hline
    $\dot{1}$ & 0 & $\onethird$ & 0 & $\onethird$ & 0 & $\onethird$ & 0 & $\onethird$ \\
    $\ddot{1}$ & 0 & 0 & 1 & 0 & 1 & 0 & 1 & 1\\
    $\dot{2}$ & $\onethird$ & 0 & 0 & $\onethird$ & 0 & $\onethird$ & 0 & $\onethird$ \\
    $\dot{3}$ & $\onethird$ & $\onethird$ & 0 & 0 & 0 & $\onethird$ & 0 & $\onethird$ \\
    $\dot{4}$ & $\onethird$ & $\onethird$ & 0 & $\onethird$ & 0 & 0 & 0 & $\onethird$ \\
  \end{tabular}
  \subcaption{}
...
dual optimal solution has all $\alpha_j^\ast = 4/3$ for $j=1,2,3,4$. 

Now we perform Phase 1, the adaptive partitioning, following the description in
Pseudocode~\ref{alg:lpr2}. To streamline the presentation, we
assume that all ties are broken in favor of lower-numbered clients, demands or facilities.
First we create one facility at each of the four sites, denoted as $\dot{1}$,
$\dot{2}$, $\dot{3}$ and $\dot{4}$ (Line~2--4, Pseudocode~\ref{alg:lpr2}). 
We then execute the ``while" loop in Line 5. This loop will have seven iterations.
Consider the first iteration. In Line 7--8 we compute $\tcc(j)$ for each
client $j=1,2,3,4$. When computing $\wtildeN_1(2)$,
facility $\dot{1}$ will get partitioned into $\dot{1}$ and $\ddot{1}$ with $\bary_{\dot{1}}=1/3$
and $\bary_{\ddot{1}} = 1$. (This will happen in Line4--7 of
Pseudocode~\ref{alg:helper}.)
Then, in Line~9 we will pick client $p=1$  and create a demand denoted as $1'$
(see Table~\ref{tbl:example_part}). Since there are no primary demands
yet, we make $1'$ a primary demand with $\wbarN(1') = \wtildeN_1(1) =
\{\dot{2}, \dot{3}, \dot{4}\}$. Notice that client $1$ is exhausted
after this iteration. 

In the second iteration we compute $\tcc(j)$ for
$j=2,3,4$ and pick client $p=2$, from which we create a new demand
$2'$. Since $\wtildeN_1(2)$ overlaps with $\wbarN(1')$, we assign $2'$
to $1'$ and set $\wbarN(2') = \wbarN(1') \cap \wtildeN(2) = \{\dot{3},
\dot{4}\}$. In the next two iterations we will pick client $p$ to be $3$ and
then $4$ and create one demand for each. These demands, denoted $3'$ and $4'$,
respectively, will be assigned to $1'$, and their neighborhoods will be set to
$\wbarN(3') = \{\dot{2}, \dot{4}\}$ and
$\wbarN(4') = \{\dot{2}, \dot{3}\}$. 
In the fifth iteration we  pick client $4$ again.
This time we have $\wtildeN_1(4) = \{\ddot{1}\}$, and this set is
disjoint from $\wbarN(1')$. So we create demand $4''$ and
make it primary, and set $\wbarN(4'') = \{\ddot{1}\}$. In the next two
iterations we create one demand for each client $2$ then $3$, denoted $2''$ and $3''$,
respectively. They both get assigned to $4''$, because their neighborhood
$\wtildeN_1(j)$ overlaps with
$\wbarN(4'')$. We also set $\wbarN(2'') = \{\ddot{1}\}$ and
$\wbarN(3'') = \{\ddot{1}\}$. 

Now that all clients are exhausted we perform Phase 2, the
augmenting phase, to construct a fractional solution in which
all demands have total connection value equal to $1$. 
We iterate through each of the seven demands created, that is
$1',2',2'',3',3'',4',4''$.
$1'$ already has a neighborhood with total
connection value of $1$, so nothing will change. 
$2'$ has $\dot{3},\dot{4}$
in its neighborhood, with total connection value of $2/3$, and
$\wtildeN(2) = \{\dot{1}\}$ at this time, so we add $\dot{1}$ into
$\wbarN(2')$ to make $\wbarN(2') = \{\dot{1}, \dot{3}, \dot{4}\}$ and
now $2'$ has total connection value of $1$. Similarly, $3'$ and $4'$
each get $\dot{1}$ added to their neighborhood and end up with total
connection value of $1$. The other three demands, namely $2'',3''$ and $4''$,
each have $\ddot{1}$ in its neighborhood so each of them has already its
total connection  value equal $1$. This completes Phase 2.
