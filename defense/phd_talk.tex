...
\begin{frame}
  ...
  \pause
  \begin{itemize}
  \item bid($j$) = avgdist($N_1(j)$) + $\alpha_j^\ast$
  \item Best bid client $p$ creates a demand
  \item Two cases, depending on $N_1(p)$
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \large{\textcolor{blue}{Best client $p$ creates demand $\nu$, to decide $N(\nu)$, two cases:}}
  \begin{itemize}
...
{\large \textcolor{blue}{Rounding: boost
    $(\bfx^\ast,\bfy^\ast)$ by $\gamma$ and use for demand reduction and adaptive partitioning, then}
