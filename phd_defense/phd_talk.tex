...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \large{
  \item Allow a reduction from FTFP to FTFL
  \item Show approximation ratio approaches 1 when all
    demands $r_j$ large
}
  \end{itemize}
\end{frame}
...
%    \node[fill=yellow!50] (label) at (0, 9.5) {split};
    ...
    \node[fill=yellow!50] (nonprimarylabel) at (8.3,8.5) {non-primary};
...
\begin{frame}
  ...
  \begin{itemize}\addtolength{\itemsep}{1\baselineskip}
  \item \large{Allow generalization of LP-rounding for UFL to apply to FTFP}
  \item \large{Take care of fault-tolerance: no facility accessible to two siblings}
  \item \large{For demand $\nu$, rounding only connects $\nu$ to $N(\nu) \cup N(\kappa)$}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \item Refined Adaptive Partition
    \begin{itemize}
    \item Two neighborhoods: close and far
    \item More intricate overlapping structure
    \end{itemize}
  \item Rounding using Byrka's algorithm for UFL
    \begin{itemize}
    \item Each primary demand has one facility open in close neigbhorhood
    \item Each non-primary demand connects to nearest neighbor if open, or its primary demand's facility
    \end{itemize}
}
  \end{itemize}
...
%%  \begin{figure}
%%    \centering
%%    \includegraphics[height=3.0in]{byrka1575.pdf}
...
\begin{frame}
  ...
  \begin{itemize}
    \addtolength{\itemsep}{1\baselineskip}
    \large{
  \item Greedy using dual-fitting is $O(\log n)$-approximation.
  \item Assuming local-charging, then dual-fitting cannot be
    better than $O(\log n / \log\log n)$.
  }
  \end{itemize}
\end{frame}

\begin{frame}
  \frametitle{Example Showing Local Charging at Least
    $O(\log n / \log\log n)$}
%%%%%%%%%%%%%%%%%%% start figure %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  \begin{tikzpicture}[scale=.5,auto]
    ...
    \node[draw,ellipse,minimum width=3cm,minimum
    height=1.2cm] (client1) at
    (6,0) {$n_1 = k^{k-1}, r_1$};
    \node[draw,ellipse,minimum width=3cm,minimum
    height=1.2cm] (client2) at
    (6,-3) {$n_2 = k^{k-2}, r_2$};
    \node[draw,ellipse,minimum width=3cm,minimum
    height=1.2cm] (client3) at
    (6,-6) {$n_3 = k^{k-3}, r_3$};
    \node[draw,ellipse,minimum width=3cm,minimum
    height=1.2cm] (clientk) at
    (6,-12) {$n_k = 1, r_k$};
    
    \node[fill=yellow!50] at (-3,-12) {\large{demands $r_1 \ll r_2 \ll \ldots \ll r_k$}};

    \draw (fac) to node {$d_1=0$} (client1);
    \draw[bend right] (fac) to (client2);
    \node (e2label) at (.8,-1.8) {$d_2 = d_1 + f_1 /
      n_2$};
    \draw[bend right] (fac) to (client3);
    \node (e3label) at (.8,-4.8) {$d_3 = d_2 + f_1 /
      n_3$};
    \draw[bend right] (fac) to  (clientk);
    \node (eklabel) at (.8,-9) {$d_k = d_{k-1} + f_1 / n_k$};
  \end{tikzpicture}
