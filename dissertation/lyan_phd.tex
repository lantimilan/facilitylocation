...
and a specification of connection from each client to an
...
as the sum of the cost to connect every client to some open
facility. The UFL problem asks for a solution that satisfies
all clients with minimum total cost. The UFL problem with
general distances has an algorithm with approximation ratio
...
relation of the two problems, UFL and Set-Cover can be found
...
the UFL problem, and we call the problem the fault-tolerant
facility placement problem ({\FTFP}). In the {\FTFP}
problem, we are also given a set of sites where zero or more
facilities could be opened, and a set of clients. Every
client specifies a demand and a client needs to be connected
...
a cost of $d_{ij}$. So an {\FTFP} instance is fully
specified by $\sitesset, \clientset$, $r_j$ for every
$j\in\clientset$, and $d_{ij}$ for every
...
as a special case (set all $r_j=1$ in {\FTFP} we get {\UFL})
, any hardness result obtained on UFL remains applicable to
FTFP. In the following, we review some well-known hardness
...
were not successful. Although {\FTFP} seems to be easier to
