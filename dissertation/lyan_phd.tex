...
\begin{abstract}
  ...
  algorithm analyzed using the dual-fitting technique gives
  an upper bound of O(logn) for approximation ratio. On the
...
and nice structure that allow a multitude of approaches to
be applicable. The problems have been studied in both
...
where zero or more facilities could be opened, and a set of
clients. Every client specifies a demand and a client needs
to be connected to several different facilities with the
number of connections equal to its demand. We are also given
the opening cost of a site, which is the cost to open one
...
{\FTFP} problem is \NP-hard, and thus preclude a promise for
polynomial time algorithms that solve the problems
exactly. Nonetheless there are polynomial time algorithms
that deliver a solution with cost only a small percentage
off from the cost of optimal solutions. These algorithms,
known as approximation algorithms and their performance
analysis, are the subject of this thesis.
...
bound of $O(\log n)$ is immediate, as the UFL problem
...
at site $i$ incurs a cost of $d_{ij}$. So an {\FTFP}
instance is fully specified by $\sitesset, \clientset$,
$r_j$ for every $j\in\clientset$, and $d_{ij}$ for every
...
\clientset$ and $\sum_{i\in\sitesset} x_{ij} = r_j$ for
...
\section{Optimization, P vs NP, and Approximation}
...
constraints. The problem also specifies a cost function that
evalutes every feasible solution to a numerical cost. For a
minimization problem, an optimal solution is a feasible
solution with cost that is no more than the cost of any
feasible solution.
...
the cost of an optimal solution. These algorithms are called
...
\begin{proof}
  ...
  this: for each element $e_j, j=1,\ldots,n$, we have a
  client $j$; and for each set $S_i, i=1,\ldots,m$, we have a
  ...
  $S_i$, whose corresponding facility $i$ is chosen to open
  in the UFL solution. Let $I$ be the set of facilities
  chosen in the UFL solution, it is easily seen that the
  corresponding sets in the Set Cover instance form a set
  cover.
\end{proof}
...
\begin{proof}
  ...
  is covered by some set $S_i$ corresponding to facility
  $i$.
\end{proof}
...
scheme (PTAS), that is, algorithms that compute a solution
with cost no more than $(1+\epsilon)$ from the optimal, for
any given constant $\epsilon > 0$, and the running time is
polynomial in the input size with $\epsilon$ being constant.
...
approximated better than $c$. We briefly mention the last
piece of hardness result, that gives a best known constant
such constant $c=1.463$. This result is also due to Guha and
Khuller.
...
\begin{proof}
  ...
  algorithm with approximation ratio $\gamma$ that is less
  than $\gamma_0 = 1.463$~\footnote{$\gamma_0$ is the
    solution to the equation $\gamma = 1 + 2/e^\gamma$.},
  then we have a polynomial time algorithm with
  approximation ratio $(1/\rho) \ln n$ for some constant
  ...
  set of clients. We repeat the iterations until all clients
  are removed.

  The metric {\UFL} instance consists of a set of facilities
  $\sitesset$ corresponding to the set $\calS$, and a set of
  clients not yet covered, that is we have one client $j$
  for each uncovered element $e_j$ in $\calU$. The distance
  $d_{ij}$ is $1$ if $e_j \in S_i$, and $3$ otherwise. We
  set $f_i = c\,n_t / k$ for some constant $c$ decided later
  to optimize our lower bound on approximation ratio.  Here
  $n_t$ is the number of clients not yet covered in the
  beginning of the $t^{th}$ iteration, and $k$ is the number
  ...
  then the ratio $\gamma$ between the UFL solution and an
  optimal integral solution for the UFL instance must be at
  least $1.463$. The solution with $k$ facilities and all
  clients connected at distance $1$ is used for optimal
  solution. The other case is that in every iteration we
  have an UFL solution that covers a large portion of
  clients, and this gives us a solution for the given Set
  Cover instance with cost (number of sets) no more than
  $(k/\rho) \ln n$, that is no more than $(1/\rho) \ln n$
  times of an optimal solution for some $\rho > 1$. This
  then implies $\NP \subseteq \DTIME(n^{O(\log\log n)})$ by
  Feige's result~\cite{Feige98}.
