...
every demand $r_j$ is no more than $|\sitesset|$. In this
case we can reduce FTFP to FTFL. The restriction on the
demands $r_j$ is essential to keeping the size of the FTFL
instance polynomially bounded by $|\sitesset|\cdot
|\clientset|$.

For the reduction we simply create $|\sitesset|$ facilities
(not opened yet) at each site. Thus, we have an FTFL
instance where every client $j$ has a demand $r_j$ and a set
of facilities that could be opened. We are then able to use
any known LP-rounding algorithm for FTFL to solve this FTFL
instance. Clearly, the solution to the FTFL instance
trivially maps to a solution to the FTFP instance. Moreover,
the approximation ratio for FTFL is preserved for
FTFP. Given that FTFL has an LP-rounding
...
\begin{theorem}
  ...
  $|\sitesset|$ and a set of clients $\clientset$, and the
  demands $r_j$ of each client $j$, let $m = |\sitesset|$ be
...
%%%%%%%%%%%%%%%%%%%
\begin{proof}
  We first solve the LP~(\ref{eqn:fac_primal}) and obtain an
  optimal fractional solution $(\bfx^\ast, \bfy^\ast)$. Then
  we apply demand reduction to obtain two instances: $\hat
  \calI$ and $\dot \calI$. For the $\hat \calI$ instance we
  already have an optimal integral solution, namely
  ...
  algorithm for {\UFL}~\footnote{We actually need more than
    ...
    satisfy this requirement.}, we obtain a solution with a
  cost no more than $c m \cdot \LP_{\smallUFL}^\ast$, where
  $\LP_{\smallUFL}^\ast$ is the cost of the optimal
  fractional solution to the {\UFL} instance. On the other
  hand, it is easy to see that $(\bfx^\ast/Q,\bfy^\ast/Q)$
  is a feasible solution to the {\UFL} instance, as we have
  $r_j \geq Q$ for every client $j$. It follows that
  \begin{equation*}
  ...
  \LP^\ast$. Let $S_1$ be this integral solution to the
  instance $\dot\calI$, and let $S_2 = (\hat\bfx,\hat\bfy)$
  be the solution to the instance $\hat\calI$. These two
  solutions $S_1$ and $S_2$, when combined, become a
  feasible integral solution to the instance $\calI$.  The
  total cost of this combined solution is no more than
  \begin{equation*}
    \cost(S_1) + \cost(S_2) \leq (cm/Q) \LP^\ast + \OPT \leq (1 +
    cm/Q) \OPT = 1 + O(m/Q) \OPT.
  \end{equation*}
  As usual, $\OPT$ denotes the cost of the optimal integral
