...
\item \emph{the Fault-Tolerant Facility Location
  ...
  demand, which is the number of facilities that it needs to
  connect to,
...
\begin{figure}[ht]
  \centering
      ...
      Adaptive Partitioning.]{An example of the first case
        ...
        $\kappa$. Facility $2$ and $4$ are in the
...
\begin{figure}[ht]
  \centering
    ...
    Adaptive Partitioning.]{An example of the second case in
      ...
      which consists of facility $1$ and $2$ in this
      example. The neighborhood $\wtildeN(p)$ keeps the rest
      facilities; that is facility $3$ and $4$ in this
      example.}
...
%%
  ...
  Partitioning.]{An example of one step in Phase 2 of
    Adaptive Partitioning. In this example, we move facility
    $2,5$ and $7$ from $\wtildeN(j)$ to $\wbarN(\nu)$ to
    make $\wbarN(\nu)$ have a total fractional value of
    $1$.}
