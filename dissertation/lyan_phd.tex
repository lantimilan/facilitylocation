...
\begin{acknowledgements}
  ...
  Chen and Dave Gomboc have read later revisions and their
  advice helped improve the presentation
  ...
  and corrected a number of grammatical errors.
...
on the approximation because it contains the Set Cover
problem as a special case. In this thesis we only consider
the metric version of the {\FTFP} problem, where the
distances $d_{ij}$ satisfy the triangle inequality, see
...
difference between {\FTFP} and {\FTFL} is that in {\FTFP} we
can open any number of facilities at a site while in {\FTFL}
we can open at most one facility at a site.
...
$d_{ij}$. Opening one facility at a site $i$ incurs a cost
of $f_i$. Making one connection from a client $j$ to a
facility at a site $i$ incurs a cost of the distance
$d_{ij}$. Thus, an {\FTFP} instance is fully specified by
$\sitesset, \clientset$, $r_j$ for every $j\in\clientset$,
$f_i$ for every $i\in\sitesset$, and $d_{ij}$ for every
$i\in\sitesset,j\in\clientset$. We consider the metric
version, that is the version in which the distances satisfy
...
y_i$ the \emph{facility cost} of the solution, and the
second term $\sum_{i \in \sitesset, j \in \clientset} d_{ij}
x_{ij}$ the \emph{connection cost} of the solution.
...
primal solution's cost to the optimal value. This is
because, assuming that the primal problem is a minimization
problem, the optimal value of the primal program is lower
bounded by the cost of any feasible dual solution.
...
fully-connected clients. The greedy algorithm for FTFP
iterates until all clients are fully-connected. In one
...
$1,2,\ldots,k$ clients for some integer $k$. For free stars,
since the greedy algorithm has not specified how to break
ties when multiple stars have the same average cost, we may
assume that the best free star is a star with just one
client. Therefore, we can find the best free star by
considering each $(i,j)$ pair for every site $i$ and every
client $j$ with $x_{ij} < y_i$. The overall best star is
...
dropping the operation would relax the constraints, and
therefore can only make the optimal value larger. The real
optimal value of (\ref{eq:star}) is then upper bounded by
the relaxed optimal value of (\ref{eq:frlp}). This allows us
to work on a relaxed program:
\begin{align}
  \label{eq:frlp}
  \textrm{maximize}\; & \frac{\sum_{j=1}^k \alpha_j}{f + \sum_{j=1}^k
    d_j}\\ \notag
...
seems the fault-tolerant requirement in both problems
presents a hurdle for primal-dual based techniques. In the
...
non-trivial star with $r_1$ replicas is $(f_1, n_1)$. Then
we have a trivial star of zero cost facility and all $n_2$
clients in group $2$ for $r_1$ replicas. The second
non-trivial star with $r_2 - r_1$ replicas is $(f_1,
n_2)$. Notice that the $r_1$ replicas of trivial stars with
...
- r_1$ replicas of the $(f_1, n_2)$ stars. The process
...
ours, we have the number of clients across groups decrease
...
We hope our work in this dissertation will assist other
researchers interested in the fault-tolerant variants of the
facility location problems to develop more insight into
their difficulties.
