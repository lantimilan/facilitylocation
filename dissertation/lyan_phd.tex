...
open one facility, or none. Each client has a demand of $1$,
meaning it needs to be connected to an open
facility. Facilities are uncapacitated, meaning a facility
can accept any number of connections from clients. We are
also given the cost of opening one facility at each site,
and the distance between a site and a client. The problem
asks us to find a set of sites to open facilities, and to
connect every client to an open facility, in a way that the
total cost is minimized. The total cost is the sum of the
cost to open facilities and the cost to connect clients to
facilities.
...
clients, by Hochbaum~\cite{Hochbaum82}. A matching lower
...
client needs to be connected to a different facility; at
each site, we can open one or more facilities. Opening
multiple facilities at the same site incurs a cost of the
opening cost for this site multiplied by the number of
facilities opened. The connection cost between a site and a
client is the number of connections times the distance
between the two, with the constraint that the number of
connections cannot exceed the number of open facilities at
that site. The FTFP problem asks for a solution with minimum
...
about the extent to which we can approximate an optimal
...
\end{theorem}
\begin{proof}
  The proof is by reduction from the Set Cover problem. In
  ...
  optimal solution for the UFL instance can only use
  connections with $d_{ij} = 1$. It is easy to see that
  given any optimal solution to the Set Cover instance, we
  can construct an optimal solution for the UFL instance,
  simply by taking the facilities whose corresponding sets
  are chosen in the set cover. On the other hand, given an
  optimal solution to the UFL instance, we can only have
  $d_{ij}=1$ connections. This implies that, for every
  client $j$, suppose it was connected to facility $i$ in
  the UFL solution, then the corresponding element $e_j$ is
  covered by the corresponding set $S_i$. Let $I$ be the set
  of facilities chosen to open in the {\UFL} solution; it is
  easily seen that the corresponding sets in the Set Cover
  instance form a set cover.
\end{proof}
...
\end{theorem}
\begin{proof}
  The reduction is also from the Set Cover problem. Suppose
  the Set Cover instance has universe $\calU =
  \{e_1,\ldots,e_n\}$ and collection of sets $\calS =
  \{S_1,\ldots,S_m\}$. Unlike the general UFL problem, we
  can no longer have edges with length $1$ and $\infty$ now,
  as the distances are constrained by the triangle
  inequality.  Our facility cost $f_i = \epsilon = 1/m^2$,
  and every facility $i$ has the same facility cost. Our
  distance $d_{ij}$ is now $1$ if $e_j \in S_i$ and $3$ if
  $e_j \notin S_i$.

  Given the construction, it is clear that any optimal
  solution for the metric UFL instance cannot use a
  connection of distance $3$, as there exists another
  solution that beats such a solution with lower cost,
  namely a solution that opens all facilities with total
  cost $m\epsilon + n\cdot 1 = m\epsilon + n$, as we have
  chosen $\epsilon = 1/m^2$. It follows that any optimal
  solution for the metric UFL instance must have all clients
  connected at distance $1$. Such a solution would then have
  a facility set corresponds to a set cover for the Set
  Cover instance.
\end{proof}
...
metric UFL problem cannot be approximated to better than
$c$-approximation. As a consequence, the metric UFL problem
cannot have polynomial-time approximation scheme (PTAS).
PTAS are algorithms that, for any constant $\epsilon > 0$,
compute a solution with cost no more than $(1+\epsilon)$
...
\end{theorem}
\begin{proof}
  The full proof can be found in~\cite{GuhaK98} and we only
  ...
  every vertex $u\in V$ has degree no more than $B$. And the
...
approximated to better than $c$, assuming $\PP \neq
...
\begin{proof}
  ...
  \{e_1,\ldots,e_n\}$ of elements and a family
  of sets $\calS = \{S_1,\ldots,S_m\}$ with every
  ...
  possible $k=1,\ldots,n$, so we can assume the knowledge of
  $k$.
  ...
  optimal {\UFL} solution's cost. The other case is that in
  every iteration we have an UFL solution that covers a
  large portion of clients. This would then give us a
  solution for the given Set Cover instance with cost
  (number of sets) no more than $(k/\rho) \ln n$, that is no
  more than $(1/\rho) \ln n$ times of an optimal solution
  for constant $\rho > 1$. This then implies $\NP \subseteq
...
%%$1.463$ unless $\NP \subseteq \DTIME(n^{O(\log\log n)})$.
  ...
  cannot have a polynomial-time algorithm with ratio less
  than $1.463$ unless $\PP = \NP$.
\end{proof}

With that, we conclude the discussion on the hardness
results for the Uncapaciated Facility Location problem
({\UFL}), with the implication that all these hardness
results carry on to our problem, the Fault-tolerant Facility
Placement problem (\FTFP) as well.
%% end of ch1
