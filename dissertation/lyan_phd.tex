...
algorithm for {\UFL}, a greedy algorithm analyzed by a
...
provide an example to illustrate the difference between
...
\label{sec: JV}
A primal-dual algorithm starts with a feasible dual solution
with all dual variables set to zero, then raises the values
of a subset of the dual variables and updates the
...
cost of no more than three times the optimal value, and
hence the JV algorithm is a $3$-approximation algorithm for
the {\UFL} problem.
...
can be solved as a {\UFL} problem.
...
be feasible, and we require that the cost of the primal
solution be bounded by the value of the possibly infeasible
dual solution. The next step is to find a suitable number
