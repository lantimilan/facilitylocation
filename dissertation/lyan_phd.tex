...
Fault-Tolerant Facility Placement problem (FTFP), as well as
those for two closely related problems, the Uncapacitated
Facility Location problem (UFL) and the Fault-Tolerant
Facility Location problem (FTFL). 
...
different.). Our goal is to open facilities at the given
sites, and connecting clients to those facilities in such a
way that all clients' demands are satisfied, and the total
cost is minimized. The difference between the problems are:
\begin{itemize}
\item UFL: All demands are $1$, that is $r_j=1$ for all
  clients $j$. Then we need no more than $1$ facility at a
  site.
\item FTFL: Some demands may be more than $1$, but each site can
  open at most $1$ facility.
...
connection cost. We shall have more to say about this in
Bifactor Analysis, Subsection~\ref{subsec:bifactor}.
...
techniques to design approximation algorithms have been
found successful on the UFL problem, as shown earlier in
...
after shrinking by $\gamma$ (divided by $\gamma$), becomes
...
allowed local moves need to be chosen carefully, as more
powerful moves allow the local optimal to be closer to the
global optimal, while restricting on a few simple moves
...
that is opened can have its cost bounded by the average cost
of the neighborhood. The connection cost of a representative
$j'$ is no more than $\alpha_{j'}$, because the complentary
slackness conditions imply $d_{ij'} \leq \alpha_{j'}$ for
any facility $i$ in $N(j')$. The connection cost for a
non-representative client $j$ can be bounded by $d_{i'j}
\leq d_{i'j'} + d_{ij'} + d_{ij}$ where $j'$ is the
...
$3\alpha_j^\ast$ because the representative has minimum
...
\begin{figure}
  ...
  LP-rounding algorithms for UFL.]{An illustration of the
    ...
    cluster. An edge is draw from a client to a neighboring
    facility. An ellipse indicates all facilities inside
    form the neighborhood of the corresponding
    representative.}
  \label{fig:STA97}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
