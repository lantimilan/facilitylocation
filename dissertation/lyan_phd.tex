...
choices, the algorithm left quite some room for
improvements. Chudak and Shmoys~\cite{ChudakS04} was the
...
connection cost is easier to analyze. With a refined
analysis, they obtained a ratio of $1+2/e =
1.736$. Sviridenko~\cite{Svi02} further improved the ratio
to $1.582$, by using a scaled version of the optimal
fractional solution, and a judicious choice of some
distribution of the scaling parameter. The rounding process
is called pipage rounding, a deterministic rounding process
that takes advantage of the concave property of some cost
function. The analysis is highly technical.

\subsubsection{Primal-dual Algorithms}
Combinatorial algorithms that do not require solving the LP
have the advantage of lower time complexity, and may turn
out to be better in approximation ratio.  Jain and
Vazirani~\cite{JainV01} initiated the primal-dual approach
to the {\UFL} problem by introducing a primal-dual algorithm
...
solution, and the cost of a feasible dual solution is a
lower bound on the cost of an optimal primal solution,
assuming the primal is a minimization problem. More on the
use of complementary slackness conditions for {\UFL} can be
found in Appendix~\ref{sec:ILP}.

A slightly different primal-dual based result is obtained by
