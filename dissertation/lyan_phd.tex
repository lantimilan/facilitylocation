...
difference between the problems are:
\begin{itemize}
...
integer program in which we use $y_i\in \{0,1\}$ to indicate
whether a facility $i\in\sitesset$ is open or not, and
$x_{ij} \in \{0,1\}$ to indicate whether a client $j$ is
connected to a facility $i$. Relaxing the integral
...
allowed local moves need to be chosen carefully: More
powerful moves allow the local optimal to be closer to the
global optimal, while restricting to a few simple moves
...
to show the algorithm can achieve a ratio of
...
clients, denoted as $C' \subseteq \clientset$ and only
...
is interesting in three aspects.
\begin{itemize}
...
$\alpha_{j'}^\ast$ value in its cluster. Summarizing, the
solution has facility cost at most $F^\ast$ and connection
cost no more than $3\sum_{j\in\clientset} \alpha_j^\ast =
3\,\LP^\ast$, where $\LP^\ast = F^\ast + C^\ast$, so it is a
$4$-approximation.
...
\\ \notag
\end{alignat}
The extra constraint $y_i \leq 1$ is what differentiates
{\FTFL} from {\FTFP}.
...
improved results on {\FTFP}. For LP-rounding algorithms, we
achieved a ratio of $1.575$, which matches the best known
LP-based ratio for {\UFL}. The LP-rounding results are
explained in Chapter~\ref{ch: lp-rounding}. For primal-dual
approaches, we provide an explanation of possible difficulty
in obtaining constant ratio using such approaches. The
primal-dual related results are in Chapter~\ref{ch:
  primal-dual}.
