...
problem. Our first technique, which we call \emph{demand
  reduction}, allows us to narrow down our attention to a
...
see Theorem~\ref{thm: reduction to polynomial}.)  Our second
technique (Section~\ref{sec: adaptive partitioning}),
adaptive partitioning, our LP-rounding algorithms
(Chapter~\ref{ch: lp-rounding}) process individual demands
of each client one by one, and thus they critically rely on
the demands being bounded polynomially in terms of
...
this set of fractional values, we still have (i) and (ii) of
Lemma~\ref{lem: polynomial demands partition}, and (iii) now
reads $\dot r_j < 2|\sitesset|$ for every client $j$.
...
other researchers on adapting the JV algorithm to {\FTFL}
...
be approximated to meet the lower bound.
