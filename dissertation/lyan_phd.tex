...
compared to LP-rounding algorithms. Primal-dual algorithms
work by making simultaneous updates to a primal solution,
which is integral, and a dual solution, and eventually
arriving at a feasible primal solution and a feasible dual
solution. The cost of these two solutions are related by the
weak duality theorem (Theorem~\ref{thm:weak_duality}) of LP
theory. It is then possible to compare the primal solution's
cost to the optimal value, since the optimal value of the
primal is lower bounded by the cost of any feasible dual
solution, assuming the primal problem is a minimization
problem.

In this chapter, we first review the primal-dual algorithm
by Jain and Vazirani~\cite{JainV01} for {\UFL} and
{\FTFL}. Next We review a greedy algorithm by Jain
{\etal.}~\cite{JainMMSV03} for {\UFL} and explain how a
similar algorithm can be used to solve {\FTFP}. We then
derive the approximation ratio for that algorithm. Lastly we
provide an example illustrating the difference between
{\UFL} and {\FTFP} when the dual-fitting analysis is used
for deriving approximation ratio.
...
({\FTFL}). However the approximation ratio is now $3\ln
...
clients now have different demands and knowing another
client connected to a few facilities does not ensure this
client having enough facilities to connect to.
...
feasible. It is easy to see that $\gamma$ is an upper bound
on the approximation ratio.

Jain {\etal}~\cite{JainMMSV03} analyzed a greedy algorithm
for {\UFL} that gives a ratio of $1.861$. The algorithm
works by repeatedly picking the most cost-effective star
until all clients are connected. A star consists of a
facility and a set of clients. The cost-effectiveness is the
cost of the star, divided by the number of clients in that
star. Clients in the just selected star connect to the
facility, and the opening cost of that facility is then set
to zero.
...
where $H_n$ is the $n^{th}$ harmonic number. Instead, we
present this result as a direct consequence of Wolsey's more
general result on Set Cover related problems.
...
2^\calU \mapsto \mathbb{N}$ which is submodular. The greedy
algorithm starts with $\calS = \emptyset$ and repeatedly
choose an element $s \in \calU$ and add to $\calS$, where
$s$ maximizes $(f(\calS \cup \{s\}) - f(\calS)) / c_s$. The
greedy algorithm has an approximation ratio of $H_d$ where
$d$ is $\max\{f(\{s\}) - f(\emptyset) \suchthat s \in
\calU\}$ is the largest possible increase in $f$ from adding
a single element.
...
r_j\}$, where $p_j$ is the number of stars in $\calS$ that
contains the client $j$. The integer $k$ in Wolsey's problem
is then set to $\sum_{j} r_j$. Since each client $j$ can
contribute no more than $r_j$ in the function $f$, having a
total of $\sum_j r_j$ guarantees every client has its share
being $r_j$.
