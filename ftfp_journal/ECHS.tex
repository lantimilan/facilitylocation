...
$\nu$ is a primary demand $\kappa$. 
Then $\Exp[C_\kappa] = \concost_{\kappa}$, 
as in the previous section, so
...
\wbarN(\nu)$. In this case we actually have $\wbarN(\kappa) =
...
0$ for $\mu\in \facilityset \setminus \wbarN(\kappa)$. 
As a result, by the algorithm (see Line~2), this case reduces to the previous one,
...
Under the assumptions listed in the last paragraph, we have
%
\begin{equation*}
	\textstyle
  \Exp[C_\nu] \leq 
...
idea of the proof is the observation that, by Pseudocode~\ref{alg:lpr3}, the
...
target facility $\phi(\kappa)$. To justify this observation, note that
we only use a
...
\begin{align*}
  \Exp[C_\nu] &\leq 
	\textstyle \sum_{r=1}^l d_ry_r\prod_{s=1}^{r-1}(1-y_s)
		+  \Exp[ d_{\phi(\kappa)\nu} |\Lambda_\nu]\cdot \prod_{s=1}^{l} (1-y_{s})
		\\
  &\leq \textstyle \Big(1 - \prod_{s=1}^l (1-y_s)\Big) \cdot \sum_{r=1}^l d_r y_r
	+  \Exp[ d_{\phi(\kappa)\nu} |\Lambda_\nu ]\cdot \prod_{s=1}^{l} (1-y_{s})
	\\
  &\leq \textstyle (1-\frac{1}{e}) \sum_{r=1}^l d_ry_r 
...
Lemma~\ref{lem:echu indirect}. This completes the proof of 
