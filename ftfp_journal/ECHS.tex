...
deal with the sub-case when $\wbarN(\kappa)\setminus \wbarN(\nu) =
\emptyset$, which is the same as $\wbarN(\kappa) \subseteq
\wbarN(\nu)$. Property (CO) implies that $\barx_{\mu\nu} =
\bary_{\mu} = \barx_{\mu\kappa}$ for every $\mu \in \wbarN(\kappa)$, so we have 
$\sum_{\mu\in\wbarN(\kappa)} \barx_{\mu\nu} = \sum_{\mu\in\wbarN(\kappa)} \barx_{\mu\kappa} = 1$, 
due to (PS.\ref{PS:one}). But also $\sum_{\mu\in\wbarN(\nu)} \barx_{\mu\nu} = 1$ and
$\barx_{\mu\nu} > 0$ for all $\mu\in \wbarN(\nu)$. Therefore
$\wbarN(\kappa) = \wbarN(\nu)$ and $C_\nu$ has exactly the same distribution as $C_\kappa$.
So this case reduces to the first case, namely
we have $\Exp[C_{\nu}] = \concost_{\nu}$, and (\ref{eqn: echs bound for connection cost}) holds.
...
The formal proof of Lemma~\ref{lem: echs expected C_nu} appears in~\cite{ChudakS04}. The main
idea of the proof the observation that the
...
(note that $\barx_{\mu_s \nu} = y_{s}$, which follows from $\barx_{\mu_s \nu} > 0$
and from (CO)), and connects $\nu$ to the nearest such open facility, if
any of them opens; otherwise $\nu$ is connected indirectly to its
target facility $\phi(\kappa)$. The intuition behind this observation is as follows:
We only use a
facility $\mu_s$ if none of $\mu_1,\ldots,\mu_{s-1}$ is open and 
$\mu_s$ opens (unconditionally) with probability $y_{s}$. The
only way that $\mu_1,\ldots,\mu_{s-1}$ can affect that
probability is if some of them belong to $\wbarN(\kappa')$, for some primary
demand $\kappa'$, and it also happens that $\mu_s \in
\wbarN(\kappa')$ as well. However, in this case, the condition that  $\mu_1,\ldots,\mu_{s-1}$
are closed actually implies that the conditional probability of
$\mu_s$ being open is larger than $y_{s}$.

Using Lemma~\ref{lem: echs expected C_nu}, we estimate the (unconditional) expected 
connection cost of $\nu$ as follows:
%
\begin{align}
  \Exp[C_\nu] &\leq 
