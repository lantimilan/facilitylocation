...
\title{LP-rounding Algorithms for the Fault-Tolerant\\
		...
		\thanks{Research supported by NSF Grant CCF-1217314.}
...
On the hardness side, it is known that it is not possible to
approximate UFL in polynomial time with ratio less than
$1.463$, provided that $\NP\not\subseteq\DTIME(n^{O(\log\log
...
be found in the full verion~\cite{YanC12}. At a high level,
...
subtle, because of complications arising in Phase 2.  This
phase does not change close neighborhoods of primary
demands, as each primary demand already contains all the
nearest facilities with total connection value equal
$1/\gamma$.  For non-primary demands, however,
...
\myparagraph{Algorithm~{\EBGS}.}  The complete algorithm
starts with solving the linear program and computing the
partitioning described earlier in this section.  Given the
partitioned fractional solution $(\barbfx, \barbfy)$ with
the desired properties, we open exactly one facility in each
...
(PD.\ref{PD1:disjoint}), as these properties together ensure
that each facility is accessible to at most one demand among
sibling demands of the same client. It is possible to show
