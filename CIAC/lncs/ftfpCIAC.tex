...
\title{LP-rounding Algorithms for the Fault-Tolerant
		...
		\\(Extended Abstract)
		\thanks{Research supported by NSF Grant CCF-1217314. Full version available in ~\cite{YanC12}.}
		}
...
\begin{abstract} 
  ...
  generalization of the Uncapacitated Facility Location
  Problem (UFL). In FTFP we are given a set of facility sites and a
  set of clients. Opening a facility at site $i$ costs $f_i$ and
  connecting client $j$ to a facility at site $i$ costs $d_{ij}$, where the 
  costs $d_{ij}$ satisfy the
  ...
  to be connected to $r_j$ different facilities. The goal is to minimize the sum of
...
gave an improved algorithm with ratio 1.5, by a combination
of LP-rounding with dual-fitting techniques.  Recently,
Li~\cite{Li11} refined the method from \cite{ByrkaA10} to
obtain ratio 1.488, which is now the best known
approximation result for UFL. Other techniques include the
primal-dual algorithm by Jain and Vazirani~\cite{JainV01},
the dual fitting method by Jain~{\etal}~\cite{JainMMSV03},
and a local search heuristic by
Arya~{\etal}~\cite{AryaGKMMP04}.  On the hardness side, it
is known that it is not possible to approximate UFL in
...
by Guha~{\etal}~\cite{GuhaMM03}, Swamy and
...
best ratio for FTFP is $3.16$ in~\cite{YanC11}, 
...
relax the integrality constraints, we obtain the following LP and its dual:
...
In the paper we fix some optimal solutions of the
LPs (\ref{eqn:fac_primal}) and (\ref{eqn:fac_dual}) that we
denote by $(\bfx^\ast, \bfy^\ast)$ and $(\bfalpha^\ast,\bfbeta^\ast)$,
respectively. We then define the
...
$C^\ast_j = \sum_{i\in\sitesset} d_{ij}x_{ij}^\ast$.  We use
...
$i,j$. As shown in~\cite{ChudakS04}, we can modify the given instance by
...
$(\bfx^\ast,\bfy^\ast)$ of LP (\ref{eqn:fac_primal}). We split this solution into two
...
$\hatr_j = \sum_{i\in\sitesset} \hatx_{ij}$ in $\hatcalI$ and
$\dotr_j = \sum_{i\in\sitesset} \dotx_{ij} = r_j - \hatr_j$ in $\dotcalI$. 
...
solution to the original instance.  Moreover, it can be shown that
{\rm (i)}
...
From these properties, we derive the following theorem (proof omitted).
...
fault-tolerance requirement. The properties are detailed
below.
%
\begin{description}
...
\end{description}
It may appear that (c1) is the same property as (CO), yet we
repeat it here as (c1) needs to hold after every iteration,
while (CO) only applies to the final partitioned fractional
solution $(\barbfx, \barbfy)$. A full description of the
algorithm is given in Pseudocode~\ref{alg:lpr2}. The two
helper routines {\NearestUnitChunk} and {\AugmentToUnit} can
be found in the full version~\cite{YanC12}. At a high level,
{\NearestUnitChunk} orders facilities in $\wtildeN(j)$ by
nonincreasing order of distance to $j$ and returns a closest
set of facilities such that the total connection value
$\tildex_{\mu j}$ is equal to $1$. {\AugmentToUnit} works
by adding facilities to $\wbarN(\nu)$ for each demand $\nu$
until every one of them has a total connection value
equal $1$.
%%%%%%%%%%%
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \While{$U\neq \emptyset$}
    \For{each $j\in U$}
    \State $\wtildeN_1(j) \assign {\NearestUnitChunk}(j, \facilityset, \tildebfx, \barbfx, \barbfy)$
...
We first describe a simple algorithm that achieves ratio $3$, in order to
illustrate how the properties of
...
\begin{proof}
  The feasibility follows from~(SI.\ref{SI:primary disjoint}) and (PD.\ref{PD:disjoint}).
...
1.736$. The basic idea to improve the ratio,
...
now we will be using a more intricate neighborhood structure.
...
the remaining facilities in $\wbarN(\nu)$ (see below). The respective average
...
Our partitioned solution $(\barbfx,\barbfy)$ must satisfy properties
...
\item{(SI')} \emph{Siblings}. For any pair $\nu,\nu'$ of different siblings we have
  \begin{enumerate}
	...
	\item \label{SI1:primary disjoint} If $\nu$ is assigned to a primary demand $\kappa$ then
 		$\wbarN(\nu')\cap \wbarclsnb(\kappa) = \emptyset$. 
...
Once all clients are exhausted, Phase~1 concludes. We then do Phase~2, the augmenting
...
The argument for (PD.\ref{PD:assign:overlap}) is a bit
subtle, because of complications arising in Phase 2.  For non-primary demands,
$\wbarN(\nu)$, for $\nu\in j$, takes all facilities in
$\wbarclsnb(\kappa)\cap \wtildeN(j)$, which might be close
to $\kappa$ but far from $j$.  As a result, facilities added
in the augmenting phase might appear in $\wbarclsnb(\nu)$,
yet they are not in $\wbarclsnb(\kappa)$. It is conceivable
that in a worst case, the facilities added in the augmenting
phase form $\wbarclsnb(\nu)$ exclusively, which will then be
disjoint from $\wbarclsnb(\kappa)$, and we may not have
(PD.\ref{PD:assign:overlap}) for demand $\nu$. Nevertheless,
we show that Property~(PD.\ref{PD:assign:overlap}) holds.
...
\myparagraph{Algorithm~{\EBGS}.} We first
solve the linear program and compute the
partitioning described earlier.  Given the
partitioned fractional solution $(\barbfx, \barbfy)$ with
the desired properties, we open exactly one facility in 
the close neighborhood of each primary demand, but
now each facility $\mu$ is chosen with probability
$\gamma\bary_{\mu}$. The facilities $\mu$
that do not belong to any set $\wbarN_{\cls}(\kappa)$ 
are opened independently with
...
\myparagraph{Analysis.}  The feasibility of our integral
solution follows from (SI.\ref{SI1:siblings disjoint}),
(SI.\ref{SI1:primary disjoint}), and
(PD.\ref{PD1:disjoint}), as these properties ensure
that siblings get connected to different facilities.
It is possible to show
that the expected facility cost of our algorithm is bounded
by $\gamma F^\ast$ and the expected connection cost can be
bounded by $C^\ast \max\{\frac{1/e+1/e^\gamma}{1-1/\gamma},
...
\bibitem{AryaGKMMP04}
V.~Arya, N.~Garg, R.~Khandekar, A.~Meyerson, K.~Munagala, and
  V.~Pandit.
 Local search heuristics for k-median and facility location problems.
 {\em SIAM J. Comput.}, 33(3):544--562, 2004.

\bibitem{ByrkaA10}
J.~Byrka and K.~Aardal.
 An optimal bifactor approximation algorithm for the metric
  uncapacitated facility location problem.
 {\em SIAM J. Comput.}, 39(6):2212--2231, 2010.

\bibitem{ByrkaGS10}
J.~Byrka, M.R.~Ghodsi, and A.~Srinivasan.
 {LP}-rounding algorithms for facility-location problems.
 {\em CoRR}, abs/1007.3611, 2010.

\bibitem{ByrkaSS10}
J.~Byrka, A.~Srinivasan, and C.~Swamy.
 Fault-tolerant facility location, a randomized dependent
  {LP}-rounding algorithm.
 In {\em Proc. 14th IPCO}, 244--257, 2010.

\bibitem{ChudakS04}
F.~Chudak and D.~Shmoys.
 Improved approximation algorithms for the uncapacitated facility
  location problem.
 {\em SIAM J. Comput.}, 33(1):1--25, 2004.

\bibitem{GuhaK98}
S.~Guha and S.~Khuller.
 Greedy strikes back: improved facility location algorithms.
 In {\em Proc. 9th SODA}, 649--657, 1998.

\bibitem{GuhaMM03}
S.~Guha, A.~Meyerson, and K.~Munagala.
 A constant factor approximation algorithm for the fault-tolerant
  facility location problem.
 {\em J. Algorithms}, 48(2):429--440, 2003.

\bibitem{gupta08}
A.~Gupta.
 Lecture notes: {{CMU}} 15-854b, spring 2008.

\bibitem{JainMMSV03}
K.~Jain, M.~Mahdian, E.~Markakis, A.~Saberi, and V.~Vazirani.
 Greedy facility location algorithms analyzed using dual fitting with
  factor-revealing {LP}.
 {\em J. ACM}, 50(6):795--824, 2003.
...
K.~Jain and V.~Vazirani.
 ...
 {\em J. ACM}, 48(2):274--296, 2001.

\bibitem{JainV03}
K.~Jain and V.~Vazirani.
 An approximation algorithm for the fault tolerant metric facility
  location problem.
 {\em Algorithmica}, 38(3):433--439, 2003.

\bibitem{Li11}
S.~Li.
 A 1.488 approximation algorithm for the uncapacitated facility
  location problem.
 In {\em Proc. 38th ICALP}, 77--88, 2011.

\bibitem{LiaoShen11}
K.~Liao and H.~Shen.
 Unconstrained and constrained fault-tolerant resource allocation.
 In {\em Proc. 17th COCOON}, 555--566, 2011.

\bibitem{ShmoysTA97}
D.~Shmoys, E.~Tardos, and K.~Aardal.
 Approximation algorithms for facility location problems (extended
  abstract).
 In {\em Proc. 29th STOC}, 265--274, 1997.

\bibitem{Svi02}
M.~Sviridenko.
 An improved approximation algorithm for the metric uncapacitated
  facility location problem.
 In {\em Proc. 9th IPCO},  240--257, 2002.

\bibitem{SwamyS08}
C.~Swamy and D.~Shmoys.
 Fault-tolerant facility location.
 {\em ACM Trans. Algorithms}, 4(4):1--27, 2008.

\bibitem{vygen05}
J.~Vygen.
 {\em Approximation Algorithms for Facility Location Problems}.
 Forschungsinst. f\"{u}r Diskrete Mathematik, 2005.

\bibitem{XuS09}
S.~Xu and H.~Shen.
 The fault-tolerant facility allocation problem.
 In {\em Proc. 20th ISAAC}, 689--698, 2009.

\bibitem{YanC11}
L.~Yan and M.~Chrobak.
 Approximation algorithms for the fault-tolerant facility placement
  problem.
 {\em Inf. Process. Lett.}, 111(11):545--549, 2011.

\bibitem{YanC12}
L.~Yan and M.~Chrobak.
{LP}-rounding Algorithms for the Fault-Tolerant Facility Placement Problem.
{\em CoRR}, abs/1205.1281, 2012.
