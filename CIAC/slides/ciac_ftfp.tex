...
\begin{frame}
  ...
  \item Combine solutions to two instances.
...
\begin{frame}
  ...
  \begin{tikzpicture}[every text node part/.style={align=center}]
    ...
    \node[ellipse, minimum width=2cm, minimum height=3.2cm, draw] (soln) at (9,0) {$\hatx_{ij} + \ddot x_{ij}$ \\ $\haty_i + \ddot y_i$};
    \node (soln1) at (9,-2) {Soln for $\calI$};

    \foreach \from/\to in {intinst/intsoln, fracinst/fracsoln, intsoln/soln, fracsoln/soln}
    \draw[->,>=latex] (\from)--(\to);
  \end{tikzpicture}
\end{frame}
%%%
