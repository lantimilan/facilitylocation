...
\begin{frame}
  ...
  neighboring facilities from near to far. This defines $\wbarN_1(j)$.
...
\begin{frame}
  \frametitle{Phase 1, Step 3}
  \begin{tikzpicture}
  \draw (1,1) ellipse (1.5cm and 2.5cm);
  \node (client) at (-1,0) {$\wtildeN(p)$};

  \draw (1,3) ellipse (1cm and 2cm);
  \node (primary) at (-1,3) {$\wbarN(\kappa)$};
  ...
  \draw[fill=blue!50] (.5,2.5) rectangle (1,3);
  \draw[fill=blue!50] (.4,1.5) rectangle (.9,2);
