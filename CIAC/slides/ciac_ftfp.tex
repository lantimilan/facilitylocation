...
\begin{frame}
  ...
  \item The Linear Program (LP) Formulation

  \item Reduction to FTFL
    \begin{itemize}
...
\begin{frame}
  ...
  \begin{minipage}{.4\linewidth}
    ...
    \node[circle,fill=green!50] (client3) at (6,3) {$3$};
    \node[circle,fill=green!50] (client2) at (6,6) {$2$};
    \node[circle,fill=green!50] (client1) at (6,11) {$1$};
    
    \foreach \from/\to in {fac11/client1,fac12/client1,fac13/client1,fac14/client1,fac11/client2,fac11/client4,fac12/client4,fac21/client3}
        \draw (\from)--(\to);

    \node (label1) at (3,-1) {Solution};
  \end{tikzpicture}
  \end{minipage}
  \vspace{.1in}
...
\begin{frame}
  ...
  \item Our approach based on optimal fractional solution to the LP.
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
  \item $y_i$  the number of facilities built at site $i$.
  \item $x_{ij}$ the number of connections from client $j$ to site
    $i$.
  \end{itemize}
  %%%%%%%%%%% 
  \begin{alignat}{3}
  ...
  %%%%%%%%%%%%
  \begin{alignat}{3}
  \text{(Dual)}\quad  \textrm{maximize}\quad \textstyle{\sum_{j\in \clientset}} r_j\, \alpha_j&\label{eqn:fac_dual}  
    \\ \notag
...
\begin{frame}
  ...
  \textbf{Goal:} Reduce a general FTFP instance to a restricted FTFP
  instance with $r_j \leq |\sitesset|$ for all clients $j$.
...
\begin{frame}
  \frametitle{Diagram for Demand Reduction}
  \begin{center}
  \begin{tikzpicture}[every text node part/.style={align=center}]
    \draw (0,0) ellipse (1cm and 2cm);
    \node (lpsoln) at (0,0) {$(x_{ij}^\ast, y_{i}^\ast)$};
    \draw (3,2) ellipse (1cm and 1.6cm);
    \node (intsoln) at (3,2) {$(\hatx_{ij}, \haty_{i})$};
    \draw (3,-2) ellipse (1cm and 1.6cm);
    \node (fracsoln) at (3,-2) {$(\dotx_{ij}, \doty_{i})$};
    \draw (8,2) ellipse (1.6cm and 1cm);
    \node (intinst) at (8,2) {$\sitesset, \clientset, f_i, d_{ij}$};
    \node (intinst1) at (8,1.5) {$\hatr_j = \sum_{i} \hatx_{ij}$};
    \draw (8,-2) ellipse (1.6cm and 1cm);
    \node (fracinst) at (8,-2) {$\sitesset, \clientset, f_i, d_{ij}$};
    \node (fracinst1) at (8,-2.5) {$\dotr_j = r_j - \hatr_j$};

    \draw[->,>=latex] (.9,1) -- (2,2);
    \draw[->,>=latex] (.9,-1) -- (2,-2);
    \draw[->,>=latex] (4,2) -- (6.4,2);
    \draw[->,>=latex] (4,-2) -- (6.4,-2);
    
    \node[above] (prop11) at (5.2,2) {feasible \\ optimal};
    \node[below] (prop12) at (5.2,2) {integral};
    \node[above] (prop21) at (5.2,-2) {feasible \\ optimal};
    \node[below] (prop22) at (5.2,-2) {fractional};

    \node[below] (inst1) at (8,.9) {instance $\hat \calI$};
    \node[below] (inst2) at (8,-3) {instance $\dot \calI$};
  \end{tikzpicture}
  \end{center}
\end{frame}
...
%%%
\begin{frame}
  \frametitle{Diagram for Demand Reduction, Cont.}
  \begin{tikzpicture}[every text node part/.style={align=center},auto]
    \node[ellipse, minimum width=3.2cm, minimum height=2cm, draw] (intinst) at (0,2) {$\sitesset, \clientset, f_i, d_{ij}$ \\ $\hatr_j = \sum_{i} \hatx_{ij}$};
    \node[below] (label1) at (0,.5) {$\hat \calI$};

    \node[ellipse, minimum width=3.2cm, minimum height=2cm, draw] (fracinst) at (0,-2) {$\sitesset, \clientset, f_i, d_{ij}$ \\ $\dotr_j = r_j - \hatr_j$};
    \node (label2) at (0,-3.5) {$\dot \calI$};

    \node[ellipse, minimum width=2cm, minimum height=3.2cm, draw] (intsoln) at (5,2) {($\hatx_{ij}, \haty_i$)};
    \node (intsoln1) at (5,0) {Soln for $\hat \calI$};
    \node[ellipse, minimum width=2cm, minimum height=3.2cm, draw] (fracsoln) at (5,-2) {$(\ddot x_{ij}, \ddot y_i)$};
    \node (fracsoln1) at (5,-4) {Soln for $\dot \calI$};
    
    \node[ellipse, minimum width=2cm, minimum height=3.2cm, draw] (soln) at (8.5,0) {$(\hatx_{ij} + \ddot x_{ij})$ \\ $(\haty_i + \ddot y_i)$};
    \node (soln1) at (8.5,-2) {Soln for $\calI$};

    % TODO: specify default arrow style
    \draw[->,>=latex] (intinst) to node {use $(\hatx_{ij},\haty_i)$} (intsoln);
    \draw[->,>=latex] (fracinst) to node {round $(\dotx_{ij},\doty_i)$ } (fracsoln);
    \foreach \from/\to in {intsoln/soln, fracsoln/soln}
    \draw[->,>=latex] (\from)--(\to);
  \end{tikzpicture}
\end{frame}
