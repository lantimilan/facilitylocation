...
%    \draw[->,>=latex,dashed,bend left] (client22) -- (client12);
    \path[->,dashed] (client22) edge[bend left=40] node[fill=yellow!50] {assigned} (client12);
    \path[->,dashed] (client21) edge[bend left=40] (client11);
  \end{tikzpicture}
...
\begin{frame}
  ...
  \begin{tikzpicture}
    ...
    \node (clslabel) at (3,3) {close neighborhood $N_{\cls}(\nu)$};
    \draw[fill=yellow!50] (8,0) ellipse (2.5cm and 2.5cm);
    \node (farlabel) at (8,3) {far neighborhood $N_{\far}(\nu)$};
