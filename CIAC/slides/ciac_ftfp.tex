...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Assumptions
      \begin{itemize}
      \item w.l.o.g. $\dotr = \poly(|\sitesset|)$
      \item w.l.o.g. $x_{ij}^\ast \in \{y_i^\ast, 0\}$
      \end{itemize}
    \item Partition
       \begin{itemize}
       \item clients into demands
       \item sites into facilities
       \item $(\bfx^\ast,\bfy^\ast)$ into $(\bar\bfx, \bar\bfy)$
       \end{itemize}
     \item Partition has a structure that allows rounding
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \item Primart demands have disjoint neighborhood
  \item Non-primary demands assigned to primary demands with overlap neighborhood
  \item Primary $\kappa$ have smaller cost than assigned non-primary $\nu$
