...
\begin{frame}
  ...
  \item {\Large \textcolor{blue}{Adaptive Partitioning}}
    ...
    \item {\large Allow rounding generalized from that for UFL}
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{tikzpicture}[auto,scale=.8,every text node part/.style={align=center}]
    \node[draw,ellipse] (lpsoln) at (0,8) {$x_{ij}^\ast, y_{i}^\ast$};
    ...
    \node[draw, ellipse] (soln) at (0,0) {$\hatx_{ij} + \ddot x_{ij}$ \\ $\haty_i + \ddot y_i$};

    \draw[->,>=latex] (lpsoln) to (intpart);
    \node (label1) at (-3,7.5) {round down};
    \draw[->,>=latex] (lpsoln) to (fracpart);
    \node (label2) at (3.2,7.5) {demand reduction};
    \draw[->,>=latex] (fracpart) to node (edge1) {adaptive\\ partition} (part);
    \draw[->,>=latex] (part) to node (edge2) {rounding} (fracsoln);
...
%% TODO: add one slide for disjoint neighborhood
...
\begin{frame}
  ...
  \vspace{.5cm}
  \begin{tikzpicture}
    \node[fill=green!75,minimum size=.6cm] (primary1) at (0,0) {$\kappa_1$};
    \node[fill=green!75,minimum size=.6cm] (primary2) at (0,-2.5){$\kappa_2$};
    \draw (2.5,0) ellipse (2cm and 1cm);
    \draw (2.5,-2.5) ellipse (2cm and 1cm);

    \node[fill=green!50,minimum size=.6cm] (demand1) at (7,0) {$\nu_1$};
    \node[fill=green!50,minimum size=.6cm] (demand2) at (7,-2.5){$\nu_2$};
    \draw (4.5,0) ellipse (2cm and 1cm);
    \draw (4.5,-2.5) ellipse (2cm and 1cm);
...
\begin{frame}
  ...
  \begin{tikzpicture}[scale=0.5]
    ...
    \node[circle,fill=green!50] (client3) at (6,2) {$r_3$};
    \node[circle,fill=green!50] (client2) at (6,7) {$r_2$};
    \node[circle,fill=green!50] (client1) at (6,12) {$r_1$};
  ...
  \begin{minipage}{.45\linewidth}
    ...
    \draw[fill=blue!25] (0,9.5) rectangle (1,11.5);
...
\begin{frame}
  ...
  \begin{tikzpicture}
    \draw[fill=yellow!50] (3,0) ellipse (1.5cm and 2.5cm);
    \node (clslabel) at (3,3) {close neighborhood $\wbarclsnb(\nu)$};
    \draw[fill=yellow!50] (8,0) ellipse (2.5cm and 2.5cm);
    \node (farlabel) at (8,3) {far neighborhood $\wbarfarnb(\nu)$};

    \node (demand) at (0,1) {demand};
    \node[fill=green!50, circle, minimum size=.6cm] at (0,0) {$\nu$};
    \node[fill=blue!50,minimum size=.6cm] (fac1) at (2,0) {};
    \node[fill=blue!50,minimum size=.6cm] at (4,0) {};
    \node[fill=blue!50,minimum size=.6cm] at (6,0) {};
    \node[fill=blue!50,minimum size=.6cm] at (8,0) {};
    \node[fill=blue!50,minimum size=.6cm] (fack) at (10,0) {};

    \draw[thick,decoration={brace,mirror,raise=3cm,amplitude=10pt},decorate] (fac1) -- (fack);
