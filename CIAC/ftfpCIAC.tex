...
\title{LP-rounding Algorithms for the Fault-Tolerant\\
 		Facility Placement Problem (Extended Abstract)~\cite{YanC12}}
...
We start with $|\sitesset|$ facilities and in each iteration of the
while loop each client causes at most one split.  We have at most
...
total number of facilities we created will be at most $|\sitesset|+
...
%
  ...
  $\nu\in j$) correspond to some choice of the $z_s$ variables (adding
  up to $1$), and the lemma follows.  \qed
\end{proof}
...
\paragraph{Algorithm~{\EGUP.}}  In Algorithm~{\EGUP}, we apply a
...
For connection cost, we shall show that the expected connection cost
