...
\usepackage{fullpage,amssymb,enumerate}
...
\end{description}
It may appear that (c1) is the same property as (CO), yet we repeat
...
loop, for each client $j \in U$ we compute a quantity called $\tcc(j)$
that represents the average distance from $j$ to the set
$\wtildeN_1(j)$ of the nearest facilities $\mu$ whose total connection
value to $j$ (the sum of $\tildex_{\mu j}$'s) equals $1$.  This set is
computed by Procedure $\NearestUnitChunk()$, which adds facilities to
$\wtildeN_1(j)$ in order of nondecreasing distance, until the total
connection value is exactly $1$. This may require splitting the last
added facility and adjusting the connection values so that properties
...
the connection values $\tildex_{\mu p}$ between client $p$ and
facilities $\mu$ in $\wtildeN(p)\cap \wbarN(\kappa)$. Note that
...
\begin{proof}
  ...
  the following linear program:\emph{
``minimize $\sum_s d_{\mu_s j}z_s$
subject to $\sum_s z_s  \ge 1$ and
 $0 \le z_s \le \tildex_{\mu_s j}$
for all $s$''}.
%
  ...
  On the other hand, we claim that $\tcc_\nu(j)$ can be thought of as
  the value of some feasible solution to this linear program, and that
  the same is true for $\concost_{\nu}$ if $\nu\in j$.  Indeed, each
  of these quantities involves some later values $\tildex_{\mu j}$,
  where $\mu$ could be one of the facilities $\mu_s$ or a new facility
  obtained from splitting. For each $s$, however, the sum of all
  values $\tildex_{\mu j}$, over the facilities $\mu$ that were split
  from $\mu_s$, cannot exceed the value $\tildex_{\mu_s j}$ at the
  time when $\eta$ was created, because splitting facilities preserves
  this sum and creating new demands for $j$ can only decrease it.
  Therefore both quantities $\tcc_\nu(j)$ and $\concost_{\nu}$ (for
  $\nu\in j$) correspond to some choice of the $z_s$ variables, and the lemma follows.  \qed
\end{proof}
...
For facility cost, by (PD.\ref{PD:disjoint}), every facility may
appear in at most one primary demand's neighborhood, and the
facilities open in Line~4--5 of Pseudocode~\ref{alg:lpr3} do not
appear in any primary demand's neighborhood. Therefore, by linearity
of expectation, the expected facility cost of Algorithm~{\ECHS} is $
\Exp[F_{\smallECHS}] = \sum_{\mu\in\facilityset} f_\mu \bary_{\mu} =
...
and Shmoys~\cite{ChudakS04}. A demand $\nu$
can either get connected directly to some facility in
...
\begin{proof}
We need to show that $D(\wbarN(\kappa)
  \setminus \wbarN(\nu), \nu) \leq \concost(\nu) +
  2\alpha_{\nu}^\ast$. There are two cases to consider.

\smallskip\noindent
{\mycase{1}}
	 There exists $\mu'\in \wbarN(\kappa) \cap
...
we get $D(\wbarN(\kappa) \setminus \wbarN(\nu), \nu) \leq
\concost_\nu + 2\alpha_{\nu}^\ast$.
...
$D(\wbarN(\kappa) \setminus \wbarN(\nu), \nu) 
  ...
  \alpha_{\kappa}^\ast + \alpha_{\nu}^\ast
	\leq \concost_\nu + 2\alpha_{\nu}^\ast$,
%
with justification similar to that in Case~1.
\qed
\end{proof}
...
\bary_{\mu_s}$ for $s = 1,\ldots,l$. We can assume that
$d_1 \le d_2 \le \ldots \le d_l$.  The
...
any of them opens; otherwise $\nu$ connects indirectly to its
target facility $\phi(\kappa)$. The intuition is that 
although the probability of opening $\mu_s$ can be affected by
opening some  of $\mu_1,\ldots,\mu_{s-1}$, this can only increase the
chances of opening $\mu_s$. (See~\cite{ChudakS04} for details.)
...
%
This gives us a bound on the connection cost,
$ \Exp[ C_{\smallECHS}] \le C^\ast +
\frac{2}{e}\LP^\ast$.
% 
Thus the total cost of our solution is 
%
...
contains all the nearest facilities with total connection $1/\gamma$.
For non-primary demands, however, $\wbarN(\nu)$, for $\nu\in j$, takes
all facilities in $\wbarclsnb(\kappa)\cap \wtildeN(j)$, which might be
close to $\kappa$ but far from $j$.  As a result, facilities added in
the augmenting phase might appear in $\wbarclsnb(\nu)$, yet they are
not in $\wbarclsnb(\kappa)$, the close neighborhood of the primary
demand $\kappa$ that $\nu$ is assigned to. It is conceivable that in a
worst case, the facilities added in the augmenting phase form
$\wbarclsnb(\nu)$ exclusively, which will then be disjoint from
$\wbarclsnb(\kappa)$, and we may not have (PD.\ref{PD:assign:overlap})
for demand $\nu$. Nevertheless, we show that
...
probabilities $\gamma \bary_{\mu}$ instead of $\bary_{\mu}$). 

We show that the expected connection cost
can be bounded by $C^\ast \max\{\frac{1/e+1/e^\gamma}{1-1/\gamma},
1+\frac{2}{e^\gamma}\}$. Properties~(PD.\ref{PD1:assign:overlap}) and
...
will connect to its target facility.  Thus
%
\begin{align*}
  \Exp[C_{\nu}] &\leq \clsdist(\nu)(1-1/e) +
...
Hence the total cost is bounded by $\max\{\gamma,
\frac{1/e+1/e^\gamma}{1-1/\gamma}, 1+\frac{2}{e^\gamma}\}\cdot
\LP^\ast$. Picking $\gamma=1.575$ we obtain:
